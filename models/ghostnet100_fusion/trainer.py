...
import pdb
import importlib
from models.ghostnet100_fusion.networkB import RGBDMH
from models.ghostnet100_fusion.loss_fn import CMFL
...
class Trainer(BaseTrainer):
    ...
    def _train_one_epoch(self, epoch, train_data_loader):
        ...
        with tqdm(total=num_train) as pbar:
            for i, batch_data in enumerate(train_data_loader):
                ...
                loss,output_prob,_,_,_ = self.compute_loss(self.network,batch_data)
    ...
    def compute_loss(self,network,img):
        ...
        op, op_rgb, op_d,op_f = network(network_input.cuda())

        loss_cmfl = self.cmfl_loss(op_rgb,op_d,target.unsqueeze(1).float())

        loss_bce = self.bce_loss(op_f,target.unsqueeze(1).float())

        loss= beta*loss_cmfl +(1-beta)*loss_bce

  

        return loss,op,op_rgb,op_d,op_f
    ...
    def test_inference(self,network,img,starter,ender):
        ...
        return loss,op,op_rgb,op_d,op_f,curr_time/24
    ...
    def test(self, test_data_loader):
        ...
        with torch.no_grad():
            for data in tqdm(test_data_loader):
                ...
                test_loss,output_prob,op_rgb,op_d,op_f,inference_time = self.test_inference(self.network,data,starter,ender
                                             )
                ...
                pred_score,pred_score_rgb,pred_score_d,pred_score_f = self._get_score_from_prob(output_prob,op_rgb,op_d,op_f)
        ...
        return test_results
    def _get_score_from_prob(self, output_prob,op_rgb,op_d,op_f):
        ...
        output_scores_f = op_f.cpu().numpy()

        return output_scores,output_scores_rgb,output_scores_d,output_scores_f
