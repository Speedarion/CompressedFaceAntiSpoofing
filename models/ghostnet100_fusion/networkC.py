...
class RGBDMH(nn.Module):
    ...
    def __init__(self, pretrained=True, num_channels=4):
        ...
        self.convA_rgb1 = nn.Conv2d(20,40,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convA_depth1 = nn.Conv2d(20,40,kernel_size=(1,1),stride=(1,1),bias=False)
        self.avgpoolA = nn.AdaptiveAvgPool2d(14)

        self.convB_rgb1 = nn.Conv2d(20,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convB_depth1 = nn.Conv2d(20,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convB_rgb2 = nn.Conv2d(40,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convB_depth2 = nn.Conv2d(40,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.avgpoolB = nn.AdaptiveAvgPool2d(7)

        self.convC_rgb1 = nn.Conv2d(20,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convC_depth1 = nn.Conv2d(20,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convC_rgb2 = nn.Conv2d(40,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.convC_depth2 = nn.Conv2d(40,80,kernel_size=(1,1),stride=(1,1),bias=False)
        self.avgpoolC = nn.AdaptiveAvgPool2d(7)
    ...
    def forward(self, img):
        ...
        #FUSION C - Summation of Features
        #Fusion 1
        #Summation of features from rgb1 and depth1 
        input1 = self.hooks['rgb1']+self.hooks['depth1']
        fuse_op=self.conv1(input1)
        ...
        #Collect features from rgb1 and depth1 to be passed into Fusion 2
        rgb1_f2 = self.convA_rgb1(self.hooks['rgb1'])
        rgb1_f2 = self.avgpoolA(rgb1_f2)
        depth1_f2 = self.convA_depth1(self.hooks['depth1'])
        depth1_f2 = self.avgpoolA(depth1_f2)

        #Fusion 2
        #Summation of features from rgb1 , depth1 , rgb2 ,depth2 and Fusion 1 Output
        input2 = fuse_op+rgb1_f2+depth1_f2+self.hooks['rgb2']+self.hooks['depth2']
        fuse_op=self.conv3(input2)
        ...
        #Collect features from rgb1,depth1,rgb2,depth2 to be passed into Fusion 3
        rgb1_f3 = self.convB_rgb1(self.hooks['rgb1'])
        rgb1_f3 = self.avgpoolB(rgb1_f3)
        depth1_f3 = self.convB_depth1(self.hooks['depth1'])
        depth1_f3 = self.avgpoolB(depth1_f3)
        rgb2_f3 = self.convB_rgb2(self.hooks['rgb2'])
        rgb2_f3 = self.avgpoolB(rgb2_f3)
        depth2_f3 = self.convB_depth2(self.hooks['depth2'])
        depth2_f3 = self.avgpoolB(depth2_f3)

        #Fusion 3
        #Summation of features from rgb1 , depth1 , rgb2 , depth2 , rgb3 ,depth 3 and Fusion 2 Output
        input3 = fuse_op+rgb1_f3+depth1_f3+rgb2_f3+depth2_f3+self.hooks['rgb3']+self.hooks['depth3']
        fuse_op=self.conv5(input3)
        ...
        #Collect features from rgb1,depth1,rgb2,depth2,rgb3,depth3 to be passed into Fusion 4
        rgb1_f4 = self.convC_rgb1(self.hooks['rgb1'])
        rgb1_f4 = self.avgpoolC(rgb1_f4)
        depth1_f4 = self.convC_depth1(self.hooks['depth1'])
        depth1_f4 = self.avgpoolC(depth1_f4)
        rgb2_f4 = self.convC_rgb2(self.hooks['rgb2'])
        rgb2_f4 = self.avgpoolC(rgb2_f4)
        depth2_f4 = self.convC_depth2(self.hooks['depth2'])
        depth2_f4 = self.avgpoolC(depth2_f4)
        #rgb3 and depth3 already with the right number of channels and image dimensions
        rgb3_f4 = self.hooks['rgb3']
        depth3_f4 = self.hooks['depth3'] 
        
        #Fusion 4
        #Summation of all features and Fusion 3 Output
        input4 = fuse_op+rgb1_f4+depth1_f4+rgb2_f4+depth2_f4+rgb3_f4+depth3_f4+self.hooks['rgb4']+self.hooks['depth4']
        fuse_op=self.conv7(input4)
